...
object Option {
  ...
  def sequence[A](a: List[Option[A]]): Option[List[A]] = a.foldRight(Some(Nil): Option[List[A]])((xOpt, accOpt) => map2(xOpt, accOpt)(_ :: _))  
  
  def traverse[A, B](a: List[A])(f: A => Option[B]): Option[List[B]] = a.foldRight(Some(Nil): Option[List[B]])((x, accOpt) => map2(f(x), accOpt)(_ :: _))
